...
--Query1 : Get a list of latest order IDs for all customers by using the max function on Order_ID column.
...
--Query2: Find suppliers who sell more than one product to Northwind Trader.
select p.SupplierID, 
...
(
    ...
    group by SupplierID 
    having count(*) > 1 
) sc on p.SupplierID = sc.SupplierID
order by p.SupplierID;

--Query3 : Create a function to get latest order date for entered customer_id.
select customerid, 
max(orderdate) 
from orders 
group by customerid;

-- Query4 : Get the top 10 most expensive products.
select * from
(
...
--Query5: Rank products by the number of units in stock in each product category.
...
--Query6: Rank customers by the total sales amount within each order date.
...
--Query7: For each order, calculate a subtotal for each Order (identified by OrderID).
select OrderID, 
...
--Query8:Sales by Year for each order. 
...
where o.ShippedDate is not null
    and od.ShippedDate between date('1996-12-24') and date('1997-09-30')
order by o.ShippedDate;

--Query9:Get Employee sales by country names.
...
---Query10:Alphabetical list of products
...
--Query11: Display the current Productlist.
select ProductID, ProductName
from products
where Discontinued = 0
order by ProductName;

--Query12:Calculate sales price for each order after discount is applied.
...
--Query13: Sales by Category: For each category, we get the list of products sold and the total sales amount.
...
--Query17:Display customer supplier by city.
